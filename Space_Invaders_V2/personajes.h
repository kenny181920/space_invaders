...
public:
    ...
    Nave(int _ancho_b, int _alto_b, int _ancho_p, int _alto_p, int _dir_bala, int _vida)
    {
        max_disp = 2;
        ancho_b = _ancho_b;
        alto_b = _alto_b;
...
{
public:
    Heroe():Nave(6, 12, 30, 20, -8, 3){}
};

class Enemigo:public Nave
{
public:
    Enemigo():Nave(6, 12, 25, 20, 8, 1){}
};
...
public:
    Producto(Nave* model,int _x,int _y,int _tipo)
    {
        m_model = model;
        x = _x;
        y = _y;
    ...
    void disparo(struct Balas disparos[], BITMAP* buffer){
        cout<<"disparo";
        if(tipo){
        crear_bala(n_disp, set_max_disp(),disparos, x, y, set_dir_bala());}
        pintar_bala(n_disp, set_max_disp(), disparos, buffer, img_bala, set_ancho_b(), set_alto_b());
        elimina_bala(n_disp, set_max_disp(), disparos, 600, 600);
    }
